  ...
  try {
    ...
    const payload = await withUser({ email: userEmail }, async (tx) => {
      const [personas, channels, pendingDrafts] = await Promise.all([
        listPersonas(tx),
        listChannels(tx),
        listActivities(tx, { statuses: ["draft"], limit: 500 }),
      ]);
...
export async function POST(req: Request) {
  try {
    ...
    const persona = await withUser({ email: userEmail }, async (tx) => {
      const created = await createPersona(tx, {
        handle: text("handle"),
      ...
      for (const raw of rawChannels) {
        ...
        channels.push(await createChannel(tx, input));
      }

      return { ...created, channels, pending_drafts: 0 };
    });
