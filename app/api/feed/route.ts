...
const CHANGELOG_API_URL = resolveChangelogApiUrl(
  process.env.CHANGELOG_API_URL?.trim() || "https://changelog-generator.up.railway.app",
);
...
function parseSource(value: string | null): FeedSource {
  if (value === "changelog") {
    return "changelog";
  }
  if (value === "all") {
...
async function fetchFeedPosts(source: FeedSource): Promise<FeedItem[]> {
  if (source === "blog") {
  ...
  if (blogResult.status === "rejected" && changelogResult.status === "rejected") {
...
function normalizeChangelogPullRequest(
  ...
  const number = Number.isInteger(numberValue) && numberValue > 0 ? numberValue : null;
