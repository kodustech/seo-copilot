-- Reply rate excluded bounces but counted autoresponders.
--
-- Every "replies" figure below filtered `reply_class <> 'bounce'` and nothing
-- else, so an out-of-office landed in the same bucket as "let's book a call".
-- The class has existed since the classifier shipped; only these aggregates
-- never used it. Nothing in the payload shape changes — `reply_classes` still
-- reports auto_reply on its own, so the volume remains visible where it belongs.
--
-- Redefinition of public.outbound_metrics, identical to
-- 20260805000000_outbound_metrics.sql except for that predicate.
...
BEGIN
  ...
  replies AS (
    ...
    WHERE rt.enrollment_id IS NOT NULL
      ...
      AND rt.first_inbound_at <  p_until
      AND COALESCE(rt.reply_class, '') NOT IN ('bounce', 'auto_reply')
      AND (p_sequence_id IS NULL OR rt.sequence_id = p_sequence_id)
  ),
  reply_agg AS (
    SELECT position,
  ...
  reps AS (
    SELECT rt.sequence_id,
           count(*) FILTER (WHERE COALESCE(rt.reply_class, '') NOT IN ('bounce', 'auto_reply')) AS replies,
