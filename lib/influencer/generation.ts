...
import { dbAdmin, type Db } from "@/lib/db";
...
export async function generateDraftsForPersona({
  ...
  const inserted = await insertActivities(db, drafts);
...
export async function runInfluencerContentCron(
  ...
  const personas = await listActivePersonas(db);
  if (!personas.length) return [];

  const lookback = new Date(now);
  lookback.setUTCDate(lookback.getUTCDate() - DEDUP_LOOKBACK_DAYS);
  const recent = await listRecentContents(db, lookback.toISOString());
  ...
  for (const persona of personas) {
    ...
    try {
      if (await hasGeneratedActivitiesSince(db, persona.id, dayStart)) {
        results.push({ ...base, skipped: true });
        continue;
      }

      const channels = await listChannelsForPersona(db, persona.id);
