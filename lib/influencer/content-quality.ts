...
const RESEARCH_PROCESS_NOTE =
  /\b(?:checked|accessed|retrieved|reviewed|read|consulted|looked at)\s+(?:from\s+|in\s+)?(?:the\s+)?(?:vendor|project|official|product)?\s*(?:page|documentation|docs?|source|site|repository|repo|material(?:s)?)\b[^.]{0,40}\b(?:on|as of)\s+(?:\d{4}-\d{2}-\d{2}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4})|\b(?:verificad[oa]|consultad[oa]|acessad[oa]|lida|le[iu]da|revisad[oa])\s+(?:n[oa]\s+)?(?:documentação|página|fonte|site|repositório|material)\b[^.]{0,40}\b(?:em|no dia)\s+(?:\d{4}-\d{2}-\d{2}|\d{1,2}[/-]\d{1,2}[/-]\d{2,4})/i;
...
const RESEARCH_ACCESS_STAMP = new RegExp(
  String.raw`${STAMP_START}(?:(?:accessed|retrieved|consulted)\s+(?:on|as of)|(?:consultad[oa]|acessad[oa])\s+(?:(?:em|no dia)|n[oa]\s+(?:site|documentação|página|fonte|repositório|material)\b[^.]{0,40}?\s+(?:em|no dia)))\s+${STAMP_DATE}`,
  "im",
);
