...
function buildShiftGoal(persona: Persona, allowed: string[]): string {
  return [
    ...
    "Start by calling browse_signals to see what Hacker News (and subreddits like LocalLLaMA or MachineLearning) are discussing today. Pick something current in your beat, open the source with fetch_url to get specifics, then write and queue ONE self-contained piece for ONE of those channels — or engage thoughtfully. For X, that is a single standalone tweet that stands on its own; never a thread or thread pieces. Queue at most one piece. Ground it in something concrete, match your voice, and do NOT repeat something you recently did. When you've done one solid thing, stop.",
  ].join("\n");
}
