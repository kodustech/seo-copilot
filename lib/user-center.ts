...
export async function getUserOverview(
  ...
  const kanbanItems = myItems.map((i) => {
    ...
    if (overdue) {
      attention.push({
        ...
        source: "kanban",
        severity: "warning",
        title: `Card atrasado: ${i.title}`,
        body: `Vencido em ${i.dueAt?.slice(0, 10)} · estágio ${i.stage}`,
        sourceId: i.id,
        link: "/kanban",
  ...
  const crmCompanies = companies.map((c) => {
    if (c.isStale) {
      attention.push({
        ...
        source: "crm",
        severity: "warning",
        title: `Conta parada: ${c.name}`,
        body: `${c.idleDays ?? "?"}d sem atividade · status ${c.status}`,
        sourceId: c.id,
        link: "/crm",
  ...
  const outreachItems = activeProspects.map((p) => {
    ...
    if (due) {
      attention.push({
        ...
        body: `Agendado para ${p.nextFollowupAt?.slice(0, 10)} · status ${p.status}`,
        sourceId: p.id,
        link: "/outreach",
  ...
  const goalItems = goals.map((g) => {
    ...
    if (atRisk) {
      attention.push({
        ...
        body: `${g.currentCount}/${g.targetCount} (${Math.round(progress * 100)}%) · ritmo esperado ${Math.round(pace * 100)}%`,
        sourceId: g.id,
        link: "/goals",
      ...
      if (j.enabled) {
        ...
        if (lastStatus === "failed") {
          attention.push({
            ...
            source: "jobs",
            severity: "error",
            title: `Job falhou: ${j.name}`,
            body: runs[0]?.error?.slice(0, 160) ?? "Última execução falhou",
            sourceId: j.id,
            link: "/jobs",
  ...
  if (replyPending > 0) {
    attention.push({
      ...
      source: "reply_radar",
      severity: "info",
      title: `${replyPending} resposta(s) aguardando no Reply Radar`,
      body: "Candidatos prontos para revisar/enviar.",
      sourceId: userEmail,
      link: "/reply-radar",
